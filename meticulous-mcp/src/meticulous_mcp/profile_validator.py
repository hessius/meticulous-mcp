...
class ProfileValidator:
    ...
    def validate(self, profile: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        # Add validation for stage limits (flow stages need pressure limits, pressure stages need flow limits)
        stage_limit_errors = self._validate_stage_limits(profile)
        errors.extend(stage_limit_errors)
        
        # Add validation for variables (emoji prefix for info, unused adjustable vars)
    ...
    def _validate_stage_limits(self, profile: Dict[str, Any]) -> List[str]:
        """Validate that stages have appropriate limits.
        
        - Flow stages MUST have a pressure limit to prevent pressure runaway
        - Pressure stages MUST have a flow limit to prevent channeling
        
        Args:
            profile: Profile dictionary to validate
            
        Returns:
            List of stage limit validation errors
        """
        errors = []
        ...
        for i, stage in enumerate(profile["stages"]):
            ...
            limit_types = [lim.get("type") for lim in limits if isinstance(lim, dict)]
            
            if stage_type == "flow":
                # Flow stages need pressure limits
                if "pressure" not in limit_types:
                    errors.append(
                        f"Stage '{stage_name}' is a flow stage but has no pressure limit. "
                        f"Flow stages MUST have a pressure limit to prevent pressure runaway. "
                        f"Add: \"limits\": [{{\"type\": \"pressure\", \"value\": 9}}]"
                    )
            elif stage_type == "pressure":
                # Pressure stages need flow limits
                if "flow" not in limit_types:
                    errors.append(
                        f"Stage '{stage_name}' is a pressure stage but has no flow limit. "
                        f"Pressure stages MUST have a flow limit to prevent channeling. "
                        f"Add: \"limits\": [{{\"type\": \"flow\", \"value\": 5}}]"
                    )
        
        return errors

    def _validate_variables(self, profile: Dict[str, Any]) -> List[str]:
        """Validate variable naming conventions and usage.
        
        - Info variables (key starts with 'info_') MUST have emoji prefix in name
        - Adjustable variables (no 'info_' prefix) must NOT have emoji prefix
        ...
        def starts_with_emoji(text: str) -> bool:
            ...
            return ord(first_char) > 0x1F300 or first_char in "☕🔧💧⚠️🎯✨🌡️⏱️📊🔥💪🎨🌊"
        
        # Collect all variable references used in stages
        used_var_keys = set()
                            ...
                            if isinstance(point, list):
                                for val in point:
                                    if isinstance(val, str) and val.startswith("$"):
                                        used_var_keys.add(val[1:])  # Remove $
                # Check limits
                limits = stage.get("limits", [])
                if isinstance(limits, list):
                    for limit in limits:
                        if isinstance(limit, dict):
                            val = limit.get("value")
                            if isinstance(val, str) and val.startswith("$"):
                                used_var_keys.add(val[1:])
                # Check exit triggers
                exit_triggers = stage.get("exit_triggers", [])
                if isinstance(exit_triggers, list):
        ...
        for var in variables:
            ...
            var_key = var.get("key", "")
            is_info = var_key.startswith("info_")
            has_emoji = starts_with_emoji(var_name)
            
            if is_info:
                # Info variables MUST have emoji prefix
                if not has_emoji:
                    errors.append(
                        f"Info variable '{var_name}' (key: {var_key}) must have an emoji prefix. "
                        f"Info variables should start with an emoji (e.g., '☕ Dose', '🔧 Filter')."
                    )
            else:
                # Adjustable variables must NOT have emoji prefix
                if has_emoji:
                    errors.append(
                        f"Adjustable variable '{var_name}' (key: {var_key}) should NOT have an emoji prefix. "
                        f"Only info variables (key starting with 'info_') should have emoji prefixes."
                    )
                
                # Adjustable variables MUST be used in at least one stage
                if var_key not in used_var_keys:
                    errors.append(
                        f"Adjustable variable '{var_name}' (key: {var_key}) is not used in any stage. "
                        f"Adjustable variables must be referenced in dynamics, limits, or exit triggers using ${var_key}."
                    )

...
# ============================================
# Stage Limits Validation Tests
# ============================================

def test_validate_flow_stage_missing_pressure_limit(validator):
    """Test that flow stages without pressure limits fail validation."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
        "stages": [
            {
                ...
                "key": "gentle_bloom",
                "type": "flow",
                "dynamics": {"points": [[0, 1.5]], "over": "time"},
    ...
    is_valid, errors = validator.validate(profile)
    assert not is_valid
    assert any("flow stage" in e.lower() and "pressure limit" in e.lower() for e in errors)


def test_validate_pressure_stage_missing_flow_limit(validator):
    """Test that pressure stages without flow limits fail validation."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
        "stages": [
            {
                ...
                "key": "main_extraction",
                "type": "pressure",
                "dynamics": {"points": [[0, 9]], "over": "weight"},
                "exit_triggers": [{"type": "weight", "value": 36, "comparison": ">=", "relative": False}],
                "limits": [],  # Missing flow limit!
            }
        ],
    }
    is_valid, errors = validator.validate(profile)
    assert not is_valid
    assert any("pressure stage" in e.lower() and "flow limit" in e.lower() for e in errors)


def test_validate_flow_stage_with_pressure_limit_passes(validator):
    """Test that flow stages with proper pressure limits pass validation."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
    ...
    is_valid, errors = validator.validate(profile)
    stage_limit_errors = [e for e in errors if "flow stage" in e.lower() and "pressure limit" in e.lower()]
    assert len(stage_limit_errors) == 0


def test_validate_pressure_stage_with_flow_limit_passes(validator):
    """Test that pressure stages with proper flow limits pass validation."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
    ...
    is_valid, errors = validator.validate(profile)
    stage_limit_errors = [e for e in errors if "pressure stage" in e.lower() and "flow limit" in e.lower()]
    assert len(stage_limit_errors) == 0
...
def test_validate_info_variable_missing_emoji(validator):
    ...
    profile = {
        ...
        "temperature": 90.0,
        "variables": [
            {"name": "Dose", "key": "info_dose", "type": "weight", "value": 18}  # Missing emoji!
        ],
    }
    is_valid, errors = validator.validate(profile)
    assert not is_valid
    assert any("info variable" in e.lower() and "emoji" in e.lower() for e in errors)


def test_validate_info_variable_with_emoji_passes(validator):
    ...
    is_valid, errors = validator.validate(profile)
    emoji_errors = [e for e in errors if "info variable" in e.lower() and "emoji" in e.lower()]
    assert len(emoji_errors) == 0


def test_validate_adjustable_variable_with_emoji_fails(validator):
    ...
    profile = {
        ...
        "temperature": 90.0,
        "variables": [
            {"name": "☕ Peak Pressure", "key": "peak_pressure", "type": "pressure", "value": 9}  # Has emoji but not info!
        ],
        "stages": [
            {
                "name": "Main",
                "key": "main",
                "type": "pressure",
                "dynamics": {"points": [[0, "$peak_pressure"]], "over": "weight"},
                "exit_triggers": [{"type": "weight", "value": 36, "comparison": ">=", "relative": False}],
                "limits": [{"type": "flow", "value": 5}],
            }
        ],
    ...
    is_valid, errors = validator.validate(profile)
    assert not is_valid
    assert any("adjustable variable" in e.lower() and "should not" in e.lower() and "emoji" in e.lower() for e in errors)


def test_validate_unused_adjustable_variable_fails(validator):
    ...
    profile = {
        ...
        "temperature": 90.0,
        "variables": [
            {"name": "Peak Pressure", "key": "peak_pressure", "type": "pressure", "value": 9}  # Not used anywhere!
        ],
        "stages": [
            {
                "name": "Main",
                "key": "main",
                "type": "pressure",
                "dynamics": {"points": [[0, 9]], "over": "weight"},  # Hardcoded, not using $peak_pressure
                "exit_triggers": [{"type": "weight", "value": 36, "comparison": ">=", "relative": False}],
                "limits": [{"type": "flow", "value": 5}],
            }
        ],
    ...
    is_valid, errors = validator.validate(profile)
    assert not is_valid
    assert any("not used" in e.lower() and "peak_pressure" in e.lower() for e in errors)


def test_validate_used_adjustable_variable_passes(validator):
    """Test that used adjustable variables pass validation."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
    ...
    is_valid, errors = validator.validate(profile)
    unused_errors = [e for e in errors if "not used" in e.lower() and "peak_pressure" in e.lower()]
    assert len(unused_errors) == 0


def test_validate_variable_used_in_limits_passes(validator):
    """Test that variables used in limits are considered used."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
        "variables": [
            {"name": "Max Flow", "key": "max_flow", "type": "flow", "value": 5}
        ],
        "stages": [
            {
                "name": "Main",
                ...
                "dynamics": {"points": [[0, 9]], "over": "weight"},
                "exit_triggers": [{"type": "weight", "value": 36, "comparison": ">=", "relative": False}],
                "limits": [{"type": "flow", "value": "$max_flow"}],  # Used here
            }
        ],
    }
    is_valid, errors = validator.validate(profile)
    unused_errors = [e for e in errors if "not used" in e.lower() and "max_flow" in e.lower()]
    assert len(unused_errors) == 0


def test_validate_variable_used_in_exit_trigger_passes(validator):
    """Test that variables used in exit triggers are considered used."""
    profile = {
        "name": "Test Profile",
        "id": "test-id",
        "temperature": 90.0,
        "variables": [
            {"name": "Target Weight", "key": "target_weight", "type": "weight", "value": 36}
        ],
        "stages": [
            {
                "name": "Main",
                ...
                "type": "pressure",
                "dynamics": {"points": [[0, 9]], "over": "weight"},
                "exit_triggers": [{"type": "weight", "value": "$target_weight", "comparison": ">=", "relative": False}],  # Used here
                "limits": [{"type": "flow", "value": 5}],
            }
        ],
    }
    is_valid, errors = validator.validate(profile)
    unused_errors = [e for e in errors if "not used" in e.lower() and "target_weight" in e.lower()]
    assert len(unused_errors) == 0
